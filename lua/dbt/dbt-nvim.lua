...
local function compile_sql(file_sql)
    ...
    local cmd = 'dbt compile --inline "' .. clean_for_compile(file_sql) .. '"'
...
local function run_operation(compiled_sql)
    ...
    local cmd =
        "dbt run-operation dbt_nvim__run_query --args \"{'sql': '"
    ...
    if handle then
        ...
        if data and data ~= "" then
            ...
            if success then
                ...
                vim.notify("Failed to decode JSON from dbt operation." .. data, vim.log.levels.ERROR)
            end
        else
...
local function get_current_buffer_query_results()
    ...
    if compiled_sql then
        compiled_sql = compiled_sql ..
            " order by (select null) offset 0 rows fetch first " .. config.limit .. " rows only "
        return compiled_sql
...
local function show_table(data, model_name)
    ...
    if config.do_create_file then
        local tmp_file = config.tmp_file_dir .. "/" .. model_name .. "_tmp.sql"
...
--         venv_path = '~/dbt_test/.venv', --path to your .venv, used for calling dbt
